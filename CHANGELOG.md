...
- a41559c: **fixed:** `PublicationMetadata` validation in case of empty `description` or empty `digi.content` for optional content pub types
...
- 312cdff: **fixed:** parse legacy `digitube-bytes`` videos as SHORT_VIDEO
...
- e4c987a: **fix:** honour publication metadata that despite being invalid did got through DiGi v1/2 validation logic
